...
export default function OutgoingCallTone(){
 ...
 const ctxRef=useRef(null),timerRef=useRef(null),nodesRef=useRef([]),ringingRef=useRef(false),callIdRef=useRef(null)
 useEffect(()=>{
  if(pathname!=='/communication')return
  let cancelled=false,poll=null,userId=null

  const stop=()=>{ringingRef.current=false;callIdRef.current=null;if(timerRef.current){clearInterval(timerRef.current);timerRef.current=null}nodesRef.current.forEach(n=>{try{n.stop?.()}catch{}try{n.disconnect?.()}catch{}});nodesRef.current=[]}
  ...
  const setWaiting=callId=>{if(callId){if(callIdRef.current!==callId){stop();callIdRef.current=callId;start()}}else stop()}
  ...
  ;(async()=>{
   ...
   channel=supabase.channel(`outgoing-ringback:${userId}`).on('postgres_changes',{event:'*',schema:'public',table:'call_sessions'},payload=>{
    ...
    if(row?.caller_id===userId){if(payload.eventType==='UPDATE'&&row.status!=='ringing')stop();else sync()}
   }).subscribe()
  })()
