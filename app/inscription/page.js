  ...
  return <main className="wr-landing">
    ...
    <div style={{display:'flex',gap:'18px',alignItems:'flex-start',justifyContent:'center',flexWrap:'wrap',width:'100%'}}><section className="wr-auth-card">
      ...
      <form className="wr-auth-form" onSubmit={mode === 'signup' ? signUp : signIn}>
        ...
        {mode === 'signup' && <label className="wr-field"><span>🌍</span><select name="countryCode" value={form.countryCode} onChange={change} required style={{width:'100%',border:0,background:'transparent',font:'inherit',outline:'none'}}>{countries.map(c=><option key={c.code} value={c.code}>{c.flag_emoji} {language==='ar'?(c.name_ar||c.name_fr):c.name_fr}</option>)}</select></label>}
        {mode === 'signup' && <label className="wr-field"><span>☎</span><input name="phone" value={form.phone} onChange={change} placeholder={tr('phone', 'Téléphone')} required /></label>}
    ...
    </section><div style={{display:'grid',gap:'12px',width:'min(420px,calc(100vw - 32px))'}}><video controls playsInline preload="metadata" src="/Ma%20vid%C3%A9o-80.mp4" style={{width:'100%',borderRadius:'18px',background:'#000',boxShadow:'0 10px 26px rgba(76,34,0,.28)'}} /></div></div>
  </main>
}
