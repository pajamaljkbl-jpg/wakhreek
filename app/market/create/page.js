...
export default function CreateBoutiquePage() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: c }, { data: v }, { data: p }, { data: pc }, { data: fx }, { data: pm }] = await Promise.all([
        ...
        supabase.from('market_payment_methods').select('code,name,payment_type,qr_asset_path,sort_order').eq('is_active', true).order('sort_order'),
      ])
      setCountries(c || [])
      setCities(v || [])
      setPlans(p || [])
  ...
  const currentPayment = useMemo(() => paymentMethods.find(p => p.code === selectedPayment), [paymentMethods, selectedPayment])
  ...
  useEffect(() => {
    if (paymentMethods.length && !paymentMethods.some(p => p.code === selectedPayment)) setSelectedPayment(paymentMethods[0].code)
  ...
  async function submit(e) {
    ...
    const { data: subscription, error: subscriptionError } = await supabase.from('subscriptions').insert({
      ...
      has_ads: hasAds,
      has_ai_agent: hasAi,
    }).select('id,boutique_id,status,billing_currency,billing_amount,amount_cfa').single()

    setSaving(false)
    if (subscriptionError) return setMessage(`Boutique créée, mais l’abonnement n’a pas pu être enregistré: ${subscriptionError.message}`)

    setCreatedBoutique(boutique)
    setCreatedSubscription(subscription)
    setMessage('Boutique enregistrée. Choisissez maintenant votre moyen de paiement ci-dessous.')
    setTimeout(() => document.getElementById('payment-space')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100)
  }

  async function selectPayment(code) {
    setSelectedPayment(code)
    if (!createdSubscription) return
    await supabase.from('subscriptions').update({
      payment_method: code,
      payment_type: paymentMethods.find(p => p.code === code)?.payment_type || 'local',
      payment_selected_at: new Date().toISOString(),
    }).eq('id', createdSubscription.id)
  }

  async function confirmPayment() {
    if (!createdSubscription || !currentPayment) return
    setPaymentBusy(true)
    setMessage('')
    const { error } = await supabase.from('subscriptions').update({
      payment_method: currentPayment.code,
      payment_type: currentPayment.payment_type || 'local',
      payment_selected_at: new Date().toISOString(),
    ...
    if (error) return setMessage(`Erreur paiement: ${error.message}`)
    setPaymentDone(true)
    setMessage(`Paiement déclaré pour ${createdBoutique?.name}. WakhReek vérifiera le paiement avant l’activation de la boutique.`)
  }
  ...
  return <AppShell>
    <main className="create-shop">
      ...
      <div className="contract-note"><strong>Principe WakhReek</strong><p>WakhReek loue l’espace de la boutique. Le vendeur reste libre de ses produits, photos, descriptions et prix de vente. Ses bénéfices ou pertes lui appartiennent.</p></div>

      {!createdSubscription && <form onSubmit={submit}>
        ...
        <section><h2>5. Gestion du stock</h2><p className="hint">Le stock reste au choix du commerçant.</p><label className="radio"><input type="radio" name="stock" checked={stockMode==='optional'} onChange={()=>setStockMode('optional')} /> Je veux gérer les quantités en stock</label><label className="radio"><input type="radio" name="stock" checked={stockMode==='hidden'} onChange={()=>setStockMode('hidden')} /> Je préfère ne pas communiquer mon stock</label></section>
        ...
        {message && <p className="message">{message}</p>}<button className="submit" disabled={saving || !selectedPlan}>{saving?'Enregistrement...':'Créer la boutique et passer au paiement'}</button>
      </form>}

      {createdSubscription && <section id="payment-space" className="payment-space">
        <div className="payment-head"><div><small>Espace paiement de la boutique</small><h2>{createdBoutique?.name}</h2></div><div className="payment-total"><small>Montant à payer</small><strong>{createdSubscription.billing_currency === 'USD' ? `$${Number(createdSubscription.billing_amount || 0).toFixed(2)} USD` : `${Number(createdSubscription.amount_cfa || 0).toLocaleString('fr-FR')} CFA`}</strong></div></div>
        <p className="hint">Choisissez un moyen de paiement. Le QR affiché ci-dessous est associé à la demande de cette boutique.</p>
        <div className="payment-methods">{paymentMethods.map(method => <button type="button" key={method.code} className={selectedPayment===method.code?'pay-method selected':''} onClick={()=>selectPayment(method.code)}><strong>{method.name}</strong><small>{method.code === 'wave' ? 'QR Wave' : method.code === 'orange_money' ? 'QR Orange Money' : 'Paiement mobile'}</small></button>)}</div>
        {currentPayment?.qr_asset_path && <div className="qr-box"><h3>{currentPayment.name}</h3><img src={currentPayment.qr_asset_path} alt={`QR ${currentPayment.name}`} /><p>Scannez ce QR pour effectuer le paiement.</p></div>}
        <label>Référence de paiement (facultatif)<input value={paymentReference} onChange={e=>setPaymentReference(e.target.value)} placeholder="Ex: référence reçue après paiement" /></label>
        <button className="submit" type="button" disabled={paymentBusy || paymentDone} onClick={confirmPayment}>{paymentDone ? 'Paiement déclaré — en vérification' : paymentBusy ? 'Enregistrement...' : 'J’ai effectué le paiement'}</button>
        <p className="activation-note">La boutique reste inactive jusqu’à la vérification du paiement par WakhReek Admin. L’Admin conserve aussi l’option d’activation gratuite.</p>
      </section>}
      {message && createdSubscription && <p className="message outside">{message}</p>}
    </main>
    <style jsx>{`
      .create-shop{max-width:980px;margin:0 auto;padding:24px;color:#172033}.create-shop>header{display:flex;gap:18px;align-items:center;margin-bottom:18px}.create-shop>header button{border:1px solid #d6deea;background:white;border-radius:9px;padding:10px 14px}.create-shop h1{margin:0;color:#075dcc}.create-shop header p{margin:5px 0;color:#667085}.contract-note{background:#fff7e8;border:1px solid #ffd48a;border-radius:12px;padding:16px;margin-bottom:18px}.contract-note strong{color:#b85a00}.contract-note p{margin:7px 0 0;line-height:1.6}form{display:grid;gap:16px}section{background:white;border:1px solid #e1e6ee;border-radius:12px;padding:20px;margin-bottom:16px}h2{font-size:18px;margin:0 0 16px;color:#075dcc}label{display:grid;gap:6px;margin:12px 0;font-size:13px;font-weight:700}input,textarea,select{border:1px solid #ccd5e2;border-radius:8px;padding:11px;font:inherit;background:white}textarea{min-height:90px;resize:vertical}.two{display:grid;grid-template-columns:1fr 1fr;gap:14px}.billing-note{display:grid;gap:4px;margin-top:14px;padding:12px;border-radius:10px;background:#eef6ff;border:1px solid #cfe4ff}.billing-note strong{color:#075dcc}.billing-note span,.hint{font-size:12px;color:#667085}.plans,.features,.payment-methods{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.plans button,.pay-method{border:2px solid #dbe3ee;background:#fff;border-radius:12px;padding:18px;text-align:left;display:grid;gap:7px}.plans button.selected,.feature.selected,.pay-method.selected{border-color:#0875e8;background:#eef6ff}.plans small,.feature small,.pay-method small{color:#667085}.plans b{color:#075dcc}.plans em{font-style:normal;font-size:11px;color:#b85a00}.feature{border:2px solid #dbe3ee;border-radius:12px;padding:14px;display:flex;gap:10px;align-items:flex-start;margin:0;cursor:pointer}.feature span{display:grid;gap:6px}.company-box{background:#f4f0ff;border:1px solid #d9caff;border-radius:12px;padding:16px}.company-box strong,.company-box b{color:#6537b8}.radio{display:flex;align-items:center;gap:9px;font-weight:500}.summary,.payment-head{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;background:#edf7ff;border-radius:10px;padding:16px;color:#075dcc}.summary div{display:grid;gap:5px}.summary small,.payment-head small{color:#667085}.summary .price,.payment-total{text-align:right}.summary strong,.payment-total strong{font-size:22px}.submit{width:100%;border:0;border-radius:10px;padding:14px;background:#0875e8;color:#fff;font-weight:900;font-size:15px}.submit:disabled{opacity:.6}.message{padding:12px;background:#fff7e8;border-radius:8px}.outside{margin-top:12px}.activation-note{color:#667085;font-size:13px}.payment-space{border:2px solid #0875e8}.payment-head{grid-template-columns:2fr 1fr;margin-bottom:16px}.payment-head h2{margin:4px 0 0}.qr-box{max-width:430px;margin:20px auto;text-align:center;background:#f8fafc;border:1px solid #e1e6ee;border-radius:16px;padding:18px}.qr-box h3{margin:0 0 14px;color:#075dcc}.qr-box img{width:100%;max-width:360px;aspect-ratio:1/1;object-fit:contain;background:white;border-radius:12px}.qr-box p{color:#667085;font-size:13px}@media(max-width:700px){.create-shop{padding:12px}.two,.plans,.features,.summary,.payment-methods,.payment-head{grid-template-columns:1fr}.summary .price,.payment-total{text-align:left}.create-shop>header{align-items:flex-start}}
    `}</style>
  </AppShell>
}
