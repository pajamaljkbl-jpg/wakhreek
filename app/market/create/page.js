...
export default function CreateBoutiquePage() {
  ...
  const monthlyPrice = useMemo(() => {
    ...
    if (isCompany || featureMode === 'ai') return selectedPlan.monthly_with_ai_cfa || selectedPlan.monthly_rent_cfa || 0
  ...
  async function submit(e) {
    ...
    setMessage(`Demande enregistrée: ${selectedPlan.name_fr} — ${monthlyPrice.toLocaleString('fr-FR')} CFA/mois. L’abonnement est en attente de paiement et de validation.`)
  }
  ...
  return <AppShell>
    <main className="create-shop">
      ...
      <form onSubmit={submit}>
        ...
        <section><h2>3. Formule de location</h2><p className="hint">Choisissez le nombre de produits que votre boutique pourra afficher.</p><div className="plans">{plans.map(p=><button type="button" key={p.code} className={selectedPlan?.code===p.code?'selected':''} onClick={()=>setPlanCode(p.code)}><strong>{p.name_fr}</strong><small>{p.product_limit == null ? 'Produits illimités' : `Jusqu’à ${p.product_limit} produits`}</small><b>{Number(p.monthly_rent_cfa || 0).toLocaleString('fr-FR')} CFA / mois</b>{p.target_segment === 'company' && <em>Entreprises / toutes les fonctionnalités</em>}</button>)}</div></section>
        <section><h2>4. Services de la formule</h2>{isCompany ? <div className="company-box"><strong>Formule Entreprise complète</strong><p>Produits illimités + publicité + agent IA + toutes les fonctionnalités WakhReek Market.</p></div> : <div className="features"><label className={featureMode==='rent'?'feature selected':''}><input type="radio" name="features" checked={featureMode==='rent'} onChange={()=>setFeatureMode('rent')} /><span><b>Location seulement</b><small>{Number(selectedPlan?.monthly_rent_cfa || 0).toLocaleString('fr-FR')} CFA / mois</small></span></label><label className={featureMode==='ads'?'feature selected':''}><input type="radio" name="features" checked={featureMode==='ads'} onChange={()=>setFeatureMode('ads')} /><span><b>Location + publicité</b><small>{Number(selectedPlan?.monthly_with_ads_cfa || 0).toLocaleString('fr-FR')} CFA / mois</small></span></label><label className={featureMode==='ai'?'feature selected':''}><input type="radio" name="features" checked={featureMode==='ai'} onChange={()=>setFeatureMode('ai')} /><span><b>Location + publicité + IA</b><small>{Number(selectedPlan?.monthly_with_ai_cfa || 0).toLocaleString('fr-FR')} CFA / mois</small></span></label></div>}</section>
