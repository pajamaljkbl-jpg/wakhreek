...
export default function SocialPage(){
 ...
 return <AppShell><main className="social">
  <div className="socialgrid"><aside className="leftcol"><form className="peoplesearch" onSubmit={searchPeople}><input value={peopleQuery} onChange={e=>setPeopleQuery(e.target.value)} placeholder={t('socialSearchPeople','Search people…')} aria-label={t('socialSearchPeople','Search people…')}/><button type="submit">{t('socialSearch','Search')}</button></form>
  {(peopleSearching||people.length>0)&&<section className="peopleresults">{peopleSearching?<p>{t('loading','Loading…')}</p>:people.map(person=><div className="person" key={person.id}>{person.avatar_url?<img src={person.avatar_url} alt=""/>:<span className="personavatar">WR</span>}<Link href={'/social/profile/'+person.id} className="personinfo"><b data-user-content data-no-translate>{person.display_name||'WakhReek'}</b>{person.country_code&&<small>{person.country_code}</small>}</Link>{user&&person.id!==user.id&&<button type="button" className={following(person.id)?'following':''} onClick={()=>toggleFollow(person.id)}>{following(person.id)?t('socialFollowing','Following'):t('socialFollow','Follow')}</button>}</div>)}</section>}{user&&<section className="friends"><h2>{t('socialFriends','Friends')}</h2>{friends.length?friends.map(friend=><Link href={'/social/profile/'+friend.id} className="friend" key={friend.id}>{friend.avatar_url?<img src={friend.avatar_url} alt=""/>:<span className="personavatar">WR</span>}<span><b data-user-content data-no-translate>{friend.display_name||'WakhReek'}</b>{friend.country_code&&<small>{friend.country_code}</small>}</span></Link>):<p>{t('socialNoFriends','No friends yet')}</p>}</section>}<section className="groups"><h2>{t('socialGroups','Groups')}</h2>{user&&<div className="groupcreate"><input value={groupName} maxLength={100} onChange={e=>setGroupName(e.target.value)} placeholder={t('socialGroupName','Group name')}/><button disabled={groupName.trim().length<2||creatingGroup} onClick={createGroup}>＋</button></div>}{groups.length?groups.map(g=><Link href={'/social/groups/'+g.id} className="groupitem" key={g.id}><span>👥</span><b data-user-content data-no-translate>{g.name}</b></Link>):<p>{t('socialNoGroups','No groups yet')}</p>}</section></aside><section className="centercol">
