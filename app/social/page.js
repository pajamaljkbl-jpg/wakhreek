...
export default function SocialPage(){
 ...
 const {t}=useI18n()
 const [ads,setAds]=useState([]),[shops,setShops]=useState([]),[posts,setPosts]=useState([]),[user,setUser]=useState(null),[draft,setDraft]=useState(''),[posting,setPosting]=useState(false),[loading,setLoading]=useState(true),[gate,setGate]=useState(false),[likes,setLikes]=useState([])
 ...
 async function load(){
  ...
  if(ids.length){const {data:l}=await supabase.from('social_likes').select('post_id,user_id').in('post_id',ids);setLikes(l||[])}else setLikes([])
  setLoading(false)
 }
 ...
 return <AppShell><main className="social">
  ...
  {loading?<div className="empty">{t('loading')}</div>:<>
   {posts.length>0&&<section className="feed">{posts.map(post=><article className="post" key={post.id}>
    ...
    <div className="postactions"><button className={likedByMe(post.id)?'liked':''} onClick={()=>toggleLike(post.id)}>{likedByMe(post.id)?'♥':'♡'} {t('socialLike').replace(/^♡\s*/,'')} {likeCount(post.id)>0&&<span>{likeCount(post.id)}</span>}</button><button onClick={locked}>{t('socialComment')}</button><button onClick={locked}>{t('socialShare')}</button></div>
   </article>)}</section>}
