...
export default function SocialPage(){
 ...
 async function load(currentUser=user){
  ...
  const [{data:a},{data:b},{data:p}]=await Promise.all([
   ...
   supabase.from('social_posts').select('id,author_id,body,media_type,media_url,boutique_id,created_at,profiles!social_posts_author_id_fkey(display_name,avatar_url),social_post_media(id,media_type,media_url,sort_order)').order('created_at',{ascending:false}).limit(20)
  ])
 ...
 async function publish(){
  ...
  const {data:post,error}=await supabase.from('social_posts').insert({author_id:user.id,body:body||null,media_type:first?.media_type||null,media_url:first?.media_url||null}).select('id').single()
  if(!error&&post){
   if(uploaded.length){const {error:mediaError}=await supabase.from('social_post_media').insert(uploaded.map(x=>({...x,post_id:post.id})));if(mediaError)console.error(mediaError)}
   setUploadProgress(100);setDraft('');setMedia([]);await load()
  }else{console.error('WakhReek Social publish error',error);alert(t('socialPublishFailed','Publication unavailable'))}
 ...
 return <AppShell><main className="social">
  ...
  {loading?<div className="empty">{t('loading')}</div>:<>
   {visiblePosts.length>0&&<section className="feed">{visiblePosts.map(post=><article className="post" id={'post-'+post.id} key={post.id}>
    ...
    {editingPost===post.id?<div className="editbox"><textarea value={editDraft} maxLength={3000} onChange={e=>setEditDraft(e.target.value)} /><div><button onClick={()=>{setEditingPost(null);setEditDraft('')}}>{t('cancel','Cancel')}</button><button onClick={()=>saveEdit(post)}>{t('save','Save')}</button></div></div>:post.body&&<p className="socialbody" data-user-content data-no-translate>{post.body}</p>}{post.author_id===user?.id&&editingPost!==post.id&&<div className="owneractions"><button onClick={()=>startEdit(post)}>{t('socialEdit','Edit')}</button><button onClick={()=>deletePost(post)}>{t('socialDelete','Delete')}</button></div>}{(post.social_post_media?.length?post.social_post_media:[post.media_url?{id:'legacy',media_type:post.media_type,media_url:post.media_url,sort_order:0}:null].filter(Boolean)).length>0&&<div className="postMediaGrid">{(post.social_post_media?.length?post.social_post_media:[{id:'legacy',media_type:post.media_type,media_url:post.media_url,sort_order:0}]).sort((a,b)=>a.sort_order-b.sort_order).map(m=>m.media_type==='video'?<video key={m.id} className="socialmedia" src={m.media_url} controls playsInline preload="metadata"/>:<img key={m.id} className="socialmedia" src={m.media_url} alt="" />)}</div>}
