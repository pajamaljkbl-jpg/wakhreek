...
export default function SocialPage(){
 ...
 return <AppShell><main className="social">
  ...
  {user&&<section className="composer"><textarea value={draft} maxLength={3000} onChange={e=>setDraft(e.target.value)} placeholder="Quoi de neuf sur WakhReek ?" /><div><small>{draft.length}/3000</small><button disabled={!draft.trim()||posting} onClick={publish}>{posting?'Publication…':'Publier'}</button></div></section>}
